...
describe("BookId", () => {
  ...
  test("toISBN() 13桁の場合", () => {
    expect(new BookId("9784167158057").toISBN()).toBe("ISBN978-4-16-715805-7");
  });

  test("toISBN() 10桁の場合", () => {
    expect(new BookId("4167158051").toISBN()).toBe("ISBN4-16-715805-1");
  });

  // 異常系
  test("不正な文字数の場合にエラーを投げる", () => {
    expect(() => new BookId("1".repeat(101))).toThrow("ISBNの長さが不正です");
    expect(() => new BookId("1".repeat(9))).toThrow("ISBNの長さが不正です");
  });
